...
- For bottle-like consumables reported as `剩返20%`, prefer fractional container stock such as `0.2 bottle` instead of storing `percent` as the live inventory unit.
- Before creating or modifying an inventory item, check for exact or similar live item names first. Only exact names should be reused automatically. If the spoken name is generic, alias-like, or has several plausible matches, ask a short clarification instead of silently mapping it.
